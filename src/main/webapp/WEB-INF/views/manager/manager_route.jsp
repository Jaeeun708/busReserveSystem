...
<div id="manager_route_content1">
	...
	<table id="dispatch_list" class="table table-hover">
		...
		<c:forEach items="${dispatchList}" var="dispatch">
			<tr>
				<td class="dispatchNo"><c:out value="${dispatch.dispatchNo}" /></td>
				<td id="dispatchSelect"><c:out value="${dispatch.busNo}" /></td>
			...
			<script>
				...
				document.getElementById('removeBtn').addEventListener('click', function() {
					var dispatchNo = getSelectedDispatchNo();
					if (dispatchNo) {
						// AJAX 요청 보내기
						deleteDispatch(dispatchNo);
					} else {
						alert('삭제할 dispatch를 선택하세요.');
					}
				});

				// 선택한 dispatch 번호를 가져오는 함수
				function getSelectedDispatchNo() {
					var selectElement = document.getElementById('dispatchSelect');
					var selectedDispatchNo = selectElement.value;
					return selectedDispatchNo;
				}

				// 삭제 함수
				function deleteDispatch(dispatchNo) {
					//AJAX 요청
					$.ajax({
						type: 'POST',
						url: '/manager/delete', // 삭제 요청을 처리하는 컨트롤러의 URL
						data: { dispatchNo: dispatchNo },
						//contentType : 'application/json; charset=UTF-8',
						success: function(response) {
							alert("ajax탐");
							// 삭제 성공 시 화면에서 해당 행을 제거
							var removedRow = document.querySelector('.dispatchNo').parentNode.parentNode;
							removedRow.remove();
						},
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var rows = document.querySelectorAll("#dispatch_list tbody tr .dispatchNo");
